...
public import Mathlib.Computability.Circuit.Encoding
public import Mathlib.Computability.Circuit.DAG

/-!
# Sharing compilation (tree circuits to DAG circuits)

This file defines a sharing-aware compilation from tree circuits to DAG circuits.

Unlike `toDAG` (which allocates a fresh node for every subcircuit), `toDAGShare` memoizes compiled
subcircuits keyed by their `Encodable.encode` value, so repeated subcircuits are compiled once and
reused.

The compiler is defined by structural recursion on `Circuit` (so it is computable), while the
...
/-- Find a cached entry whose circuit encodes to `key`.

We return membership evidence so the caller can use invariants about the cache. -/
def find? (c : Cache G n N) (key : Nat) :
    Option (Σ' c' : Circuit G n, Σ' i : Fin N, (c', i) ∈ c ∧ Encodable.encode c' = key) :=
  match c with
  | [] => none
  | (c', i) :: cs =>
...
/-- Append a node, lifting the cache and inserting the new circuit result. -/
def snoc (s : State G n) (nd : DAGNode G n s.dag.N) (c : Circuit G n) : State G n :=
  let dag' := s.dag.snoc nd
  let cache' : Cache G n dag'.N := (c, Fin.last s.dag.N) :: Cache.liftSucc s.cache
  ⟨dag', cache'⟩

omit [∀ k, Encodable (G k)] in
@[simp] theorem snoc_dag (s : State G n) (nd : DAGNode G n s.dag.N) (c : Circuit G n) :
...
def StateOk (s : State G n) : Prop :=
  ∀ {c : Circuit G n} {i : Fin s.dag.N}, (c, i) ∈ s.cache → ∀ x, s.dag.evalAt x i.1 i.2 = eval c x
...
theorem stateOk_snoc (s : State G n) (hs : StateOk (G := G) (n := n) s)
    (nd : DAGNode G n s.dag.N) (c : Circuit G n)
    (hout : ∀ x,
  ...
  · rcases List.mem_map.1 hi_tail with ⟨⟨c0, i0⟩, hi0, hEq⟩
    ...
    have hpres :
      ...
      simpa [State.snoc_dag] using
        DAG.evalAt_snoc_of_lt (d := s.dag) (nd := nd) (x := x) (i := i0.1) (hi := i0.2)
...
structure BuildResult (c : Circuit G n) (s : State G n) : Type where
  state : State G n
  pref : DAG.Prefix s.dag state.dag
  out : Fin state.dag.N
  ok : StateOk (G := G) (n := n) state
  out_correct : ∀ x, state.dag.evalAt x out.1 out.2 = eval c x

/-- Result of compiling children `f : Fin k → Circuit G n` in order, starting from `s`. -/
structure ChildrenResult {k : Nat} (f : Fin k → Circuit G n) (s : State G n) : Type where
  state : State G n
  pref : DAG.Prefix s.dag state.dag
...
def buildChildren :
  ...
  | 0, _, _, s, hs =>
      ⟨s, DAG.Prefix.refl s.dag, Fin.elim0, hs, by intro i x; exact Fin.elim0 i⟩
  | k + 1, f, childBuild, s, hs =>
      ...
      have pref : DAG.Prefix s.dag rt.state.dag := DAG.Prefix.trans r0.pref rt.pref
...
/-- Sharing-aware compilation of a circuit into a DAG, starting from a state. -/
def buildFrom (c : Circuit G n) :
    ∀ s : State G n, StateOk (G := G) (n := n) s → BuildResult (G := G) (n := n) c s :=
  fun s hs =>
    match c with
    | .input i =>
        let key := Encodable.encode (Circuit.input (G := G) i)
        match Cache.find? s.cache key with
        | some ⟨c', idx, h⟩ => by
            have hmem : (c', idx) ∈ s.cache := h.1
            have henc : Encodable.encode c' = key := h.2
            have hc : c' = Circuit.input (G := G) i := by
              exact Encodable.encode_injective (by simpa [key] using henc)
            refine ⟨s, DAG.Prefix.refl s.dag, idx, hs, ?_⟩
            intro x
            simpa [hc] using hs (c := c') (i := idx) hmem x
        | none => by
            let s' := State.snoc (G := G) (n := n) s (.input i) (Circuit.input (G := G) i)
            have pref : DAG.Prefix s.dag s'.dag := DAG.Prefix.snoc (d := s.dag) (nd := .input i)
            have hout :
                ∀ x,
                  s'.dag.evalAt x s.dag.N (Nat.lt_succ_self _) =
                    eval (Circuit.input (G := G) i) x := by
              intro x
              simp [s', State.snoc, DAG.evalAt_snoc_last, eval]
            have ok : StateOk (G := G) (n := n) s' :=
              stateOk_snoc (G := G) (n := n) s hs (.input i) (Circuit.input (G := G) i) hout
            refine ⟨s', pref, Fin.last s.dag.N, ok, ?_⟩
            intro x
            simpa using hout x
    | .const b =>
        let key := Encodable.encode (Circuit.const (G := G) (n := n) b)
        match Cache.find? s.cache key with
        | some ⟨c', idx, h⟩ => by
            have hmem : (c', idx) ∈ s.cache := h.1
            have henc : Encodable.encode c' = key := h.2
            have hc : c' = Circuit.const (G := G) (n := n) b := by
              exact Encodable.encode_injective (by simpa [key] using henc)
            refine ⟨s, DAG.Prefix.refl s.dag, idx, hs, ?_⟩
            intro x
            simpa [hc] using hs (c := c') (i := idx) hmem x
        | none => by
            let s' :=
              State.snoc (G := G) (n := n) s (.const b) (Circuit.const (G := G) (n := n) b)
            have pref : DAG.Prefix s.dag s'.dag := DAG.Prefix.snoc (d := s.dag) (nd := .const b)
            have hout :
                ∀ x,
                  s'.dag.evalAt x s.dag.N (Nat.lt_succ_self _) =
                    eval (Circuit.const (G := G) (n := n) b) x := by
              intro x
              simp [s', State.snoc, DAG.evalAt_snoc_last, eval]
            have ok : StateOk (G := G) (n := n) s' :=
              stateOk_snoc (G := G) (n := n) s hs (.const b) (Circuit.const (G := G) (n := n) b)
                hout
            refine ⟨s', pref, Fin.last s.dag.N, ok, ?_⟩
            intro x
            simpa using hout x
    | .gate (k := k) g f =>
        let key := Encodable.encode (Circuit.gate (G := G) (n := n) (k := k) g f)
        match Cache.find? s.cache key with
        | some ⟨c', idx, h⟩ => by
            have hmem : (c', idx) ∈ s.cache := h.1
            have henc : Encodable.encode c' = key := h.2
            have hc : c' = Circuit.gate (G := G) (n := n) (k := k) g f := by
              exact Encodable.encode_injective (by simpa [key] using henc)
            refine ⟨s, DAG.Prefix.refl s.dag, idx, hs, ?_⟩
            intro x
            simpa [hc] using hs (c := c') (i := idx) hmem x
        | none => by
            ...
            let s' :=
              State.snoc (G := G) (n := n) ch.state nd
                (Circuit.gate (G := G) (n := n) (k := k) g f)
            have prefKids : DAG.Prefix s.dag dagKids := ch.pref
            have pref : DAG.Prefix s.dag s'.dag :=
              DAG.Prefix.trans prefKids (DAG.Prefix.snoc (d := dagKids) (nd := nd))
            ...
            have hout :
                ∀ x,
                  s'.dag.evalAt x dagKids.N (Nat.lt_succ_self _) =
                    eval (Circuit.gate (G := G) (n := n) (k := k) g f) x := by
              intro x
              -- Use the semantic description of the newly appended node.
              simp [s', State.snoc, dagKids, nd, DAG.evalAt_snoc_last, eval, hchildren]
            have ok : StateOk (G := G) (n := n) s' :=
              stateOk_snoc (G := G) (n := n) ch.state ch.ok nd
                (Circuit.gate (G := G) (n := n) (k := k) g f) hout
...
/-- Sharing-aware compilation from tree circuits to DAG circuits. -/
def toDAGShare (c : Circuit G n) : DAGCircuit G n :=
...
theorem eval_toDAGShare (c : Circuit G n) (x : Fin n → Bool) :
    (toDAGShare (G := G) (n := n) c).eval x = eval c x := by
  let s0 : State G n := State.empty G n
  let r := buildFrom (G := G) (n := n) c s0 stateOk_empty
  simpa [toDAGShare, DAGCircuit.eval, s0, r] using r.out_correct x
...
/-- Sharing-aware compilation from tree circuits to DAG circuits. -/
def toDAGShare (c : Circuit G n) : DAGCircuit G n :=
  Share.toDAGShare (G := G) (n := n) c

theorem eval_toDAGShare (c : Circuit G n) (x : Fin n → Bool) :
    (toDAGShare (G := G) (n := n) c).eval x = eval c x :=
  Share.eval_toDAGShare (G := G) (n := n) c x
