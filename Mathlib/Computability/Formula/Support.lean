...
This file defines the (finite) set of input variables used by a formula, and proves basic lemmas.

For tree formulas, the support is computed structurally:
- an input wire uses exactly that variable
- constants use no variables
- a gate uses the union of the supports of its children

The key semantic lemma is: if two assignments agree on the support, the formula evaluates to the
same Boolean value.
-/
...
/-- The set of input variables used by a formula. -/
def support : Formula G n → Finset (Fin n)
  | .input i => {i}
  | .const _ => ∅
  | .gate (k := k) _ f =>
...
@[simp] theorem support_gate {k : Nat} (g : G k) (f : Fin k → Formula G n) :
    support (gate (G := G) (n := n) g f)
      = (Finset.univ : Finset (Fin k)).biUnion (fun i => support (f i)) :=
  rfl

theorem support_mapGate {H : Nat → Type} (φ : GateHom G H) (c : Formula G n) :
...
theorem support_mapInputs {m n : Nat} (c : Formula G n) (ρ : Fin n → Fin m) :
...
theorem support_subst {m n : Nat} (c : Formula G n) (σ : Fin n → Formula G m) :
...
theorem support_child_subset {k : Nat} (g : G k) (f : Fin k → Formula G n) (i : Fin k) :
...
theorem eval_eq_of_eqOn_support_finset [GateEval G] (c : Formula G n) {x y : Fin n → Bool}
...
theorem eval_eq_of_eqOn_support [GateEval G] (c : Formula G n) {x y : Fin n → Bool}
